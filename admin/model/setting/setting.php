...
class ModelSettingSetting extends Model {
	...
	public function addSettingValue($code = '', $key = '', $value = '', $store_id = 0) {
		if (!is_array($value)) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "setting SET `value` = '" . $this->db->escape(json_encode($value)) . "', serialized = '1', `code` = '" . $this->db->escape($code) . "', `key` = '" . $this->db->escape($key) . "', store_id = '" . (int)$store_id . "'");
		}
	}
}
