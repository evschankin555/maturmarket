...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		if (isset($data['category_city'])) {
			foreach ($data['category_city'] as $city_id) {
				if (!$city_id){
					continue;
				}
				$this->db->query("INSERT INTO " . DB_PREFIX . "category_to_city SET category_id = '" . (int)$category_id . "', city_id = '" . (int)$city_id . "'");
			}
		}
	...
	public function editCategory($category_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "category_to_city WHERE category_id = '" . (int)$category_id . "'");
	...
	public function addCategoryMathings($data){
		$this->db->query("INSERT INTO " . DB_PREFIX . "category_matching_for_import SET manufacturer_id = '" . (int)$data['manufacturer_id'] . "', category_id = '" . (int)$data['category_id'] . "', import_name = '" . $this->db->escape($data['import_name']) . "'");
	...
	public function addCategoryFilters($category_id, $filter_id) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category_filter SET category_id = '" . (int)$category_id . "', filter_id = '" . (int)$filter_id . "'");

		return $this->db->getLastId();
	}

	public function addCategoryToCity($category_id, $city_id) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category_to_city SET category_id = '" . (int)$category_id . "', city_id = '" . (int)$city_id . "'");
	...
	public function getCategoryCities($category_id) {
		$category_city_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_to_city WHERE category_id = '" . (int)$category_id . "'");

		foreach ($query->rows as $result) {
			$category_city_data[] = $result['city_id'];
		}

		return $category_city_data;
	}
