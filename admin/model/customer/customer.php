...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', city_id = '" . (int)$address['city_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : json_encode(array())) . "'");
	...
	public function getAddress($address_id) {
		$address_query = $this->db->query("SELECT *, c.name as city_name FROM " . DB_PREFIX . "address a INNER JOIN " . DB_PREFIX . "city c on c.city_id = a.city_id WHERE address_id = '" . (int)$address_id . "'");
