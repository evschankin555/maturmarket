...
class ModelExtensionReportCustomer extends Model {
	...
	public function getByPeriodWithoutOrders($data = array()) {
		$sql = "select cus.customer_id, CONCAT(cus.firstname, ' ', cus.lastname) AS customer, cus.telephone, cgd.name AS customer_group, cus.status, last_order_date, orders_count, orders_total from " . DB_PREFIX . "customer cus left join (select o.customer_id, max(o.date_added) as last_order_date, count(o.order_id) as orders_count, sum(o.total) as orders_total from " . DB_PREFIX . "order o where o.order_status_id > '0' and o.order_status_id <> 7 group by o.customer_id) orders on cus.customer_id = orders.customer_id inner join " . DB_PREFIX . "customer_group_description cgd on cus.customer_group_id = cgd.customer_group_id and cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		
		$implode = array();

		if (!empty($data['filter_customer_group_id'])) {
			$implode[] = "cus.customer_group_id = '" . $this->db->escape($data['filter_customer_group_id']) . "'";
		}

		if (!empty($data['filter_period_without_orders'])) {
			$implode[] = "DATEDIFF(CURRENT_DATE, DATE(ifnull(last_order_date, '1000-01-01'))) >= '" . $this->db->escape($data['filter_period_without_orders']) . "'";
		}

		if (!empty($data['filter_orders_count'])) {
			$implode[] = "orders_count >= '" . $this->db->escape($data['filter_orders_count']) . "'";
		}
	...
	public function getTotalByPeriodWithoutOrders($data = array()) {
		$sql = "SELECT COUNT(DISTINCT cus.customer_id) AS total from " . DB_PREFIX . "customer cus left join (select o.customer_id, max(o.date_added) as last_order_date, count(o.order_id) as orders_count, sum(o.total) as orders_total from " . DB_PREFIX . "order o where o.order_status_id > '0' and o.order_status_id <> 7 group by o.customer_id) orders on cus.customer_id = orders.customer_id inner join " . DB_PREFIX . "customer_group_description cgd on cus.customer_group_id = cgd.customer_group_id and cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
