...
class ModelExtensionDashboardSale extends Model {
	...
	public function getWeekSales() {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' and order_status_id <> 7 and WEEK(date_added, 1) = WEEK(NOW(), 1) and YEAR(date_added) = YEAR(NOW())";
	...
	public function getMonthSales() {
		$sql = "SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' and order_status_id <> 7 and MONTH(date_added) = MONTH(NOW()) and YEAR(date_added) = YEAR(NOW())";
