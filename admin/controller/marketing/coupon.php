...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['coupons'][] = array(
				...
				'once_in_group' => $result['once_in_group'],
		...
		$data['sort_once_in_group'] = $this->url->link('marketing/coupon', 'user_token=' . $this->session->data['user_token'] . '&sort=once_in_group' . $url, true);
	...
	protected function getForm() {
		...
		if (isset($this->request->post['limit_for_percent'])) {
			$data['limit_for_percent'] = $this->request->post['limit_for_percent'];
		} elseif (!empty($coupon_info)) {
			$data['limit_for_percent'] = $coupon_info['limit_for_percent'];
		} else {
			$data['limit_for_percent'] = '';
		}
		...
		if (isset($this->request->post['once_in_group'])) {
			$data['once_in_group'] = $this->request->post['once_in_group'];
		} elseif (!empty($coupon_info)) {
			$data['once_in_group'] = $coupon_info['once_in_group'];
		} else {
			$data['once_in_group'] = '';
		}
