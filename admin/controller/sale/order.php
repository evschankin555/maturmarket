...
class ControllerSaleOrder extends Controller {
    ...
    protected function getList() {
		...
		$data['sort_shipment_date'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.shipment_date' . $url, true);
    ...
    public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['shipment_date'] = $order_info['shipment_date'] ? date_create_from_format("Y-m-d", $order_info['shipment_date'])->format("d.m.Y") : ''; 
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['order_without_price'] = $this->url->link('sale/order/order_without_price', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
			$data['change_readiness'] = str_replace('&amp;', '&', $this->url->link('sale/order/change_readiness', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . (int)$this->request->get['order_id'] . '&change_to=' . ($order_info['readiness'] == 1 ? '0' : '1'), true));
			...
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['payment_method'] = $order_info['payment_method'];
			if (isset($order_info['shipment_date'])){
				$data['shipment_date'] = date($this->language->get('date_format_short'), strtotime($order_info['shipment_date']));
			}
			...
			if ($order_info['shipping_address_format']) {
				...
				// $format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
				$format = '<b>{city}</b>, {address_1}' . "\n" . '{address_2}' . "\n" . 'Контактное лицо:&nbsp;{firstname} {lastname}';
			}
			...
			if ($order_info['readiness'] == 1) {
				$data['button_change_readiness'] =  $this->language->get('button_unset_readiness');
				$data['button_change_readiness_class'] = 'btn-default';
			} else {
				$data['button_change_readiness'] =  $this->language->get('button_set_readiness');
				$data['button_change_readiness_class'] = 'btn-info';
			}
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'shipment_date'    => $order_info['shipment_date'],
	...
	public function order_without_price() {
		$this->load->language('sale/order');
		...
		$this->response->setOutput($this->load->view('sale/order_without_price', $data));
	}
	...
	public function change_readiness() {
		$this->load->language('sale/order');
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if (isset($this->request->get['order_id'])) {
				$order_id = $this->request->get['order_id'];
				$changeTo = $this->request->get['change_to']; // 0 - not ready, 1 - ready

				$this->load->model('sale/order');

				$readiness = $this->model_sale_order->changeReadiness($order_id, $changeTo);
