...
class ControllerExtensionDashboardSale extends Controller {
	...
	public function dashboard() {
		...
		*/
		...
		$sale_week = $this->model_extension_dashboard_sale->getWeekSales();

		if ($sale_week > 1000000000000) {
			$data['week_total'] = round($sale_week / 1000000000000, 1) . 'T';
		} elseif ($sale_week > 1000000000) {
			$data['week_total'] = round($sale_week / 1000000000, 1) . 'B';
		} elseif ($sale_week > 1000000) {
			$data['week_total'] = round($sale_week / 1000000, 1) . 'M';
		} elseif ($sale_week > 1000) {
			$data['week_total'] = round($sale_week / 1000, 1) . 'K';
		} else {
			$data['week_total'] = round($sale_week);
		}

		$sale_month = $this->model_extension_dashboard_sale->getMonthSales();

		if ($sale_month > 1000000000000) {
			$data['month_total'] = round($sale_month / 1000000000000, 1) . 'T';
		} elseif ($sale_month > 1000000000) {
			$data['month_total'] = round($sale_month / 1000000000, 1) . 'B';
		} elseif ($sale_month > 1000000) {
			$data['month_total'] = round($sale_month / 1000000, 1) . 'M';
		} elseif ($sale_month > 1000) {
			$data['month_total'] = round($sale_month / 1000, 1) . 'K';
		} else {
			$data['month_total'] = round($sale_month);
		}
