...
class ControllerExtensionDashboardActivity extends Controller {
	...
	public function dashboard() {
		...
		foreach ($results as $result) {
			$dataObject = json_decode($result['data'], true);
			$comment = vsprintf($this->language->get('text_activity_' . $result['key']), $dataObject);
			...
			$data['activities'][] = array(
				...
				'customer_purchases' => isset($dataObject['customer_id']) ? $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&filter_customer=' . $dataObject['name'] , true) : ''
			);
		}
