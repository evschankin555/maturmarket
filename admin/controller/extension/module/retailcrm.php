...
class ControllerExtensionModuleRetailcrm extends Controller
{
    ...
    public function customer_edit($route, $customer)
    {
        ...
        $customer_group = $this->model_customer_customer_group->getCustomerGroup($customer['customer_group_id']);
        $MASTER_CUSTOMER_GROUP_NAME = 'Мастер';

        if ($customer_group['name'] == $MASTER_CUSTOMER_GROUP_NAME) {
            $customer['users_group'] = 'master';
        } else {
            $customer['users_group'] = 'default';
        }
    ...
    public function export()
    {
        ...
        foreach ($customers as &$customer) {
            ...
            foreach ($customer_groups as $group) {
                if ($group['customer_group_id'] == $customer['customer_group_id']){
                    if ($group['name'] == $MASTER_CUSTOMER_GROUP_NAME) {
                        $customer['users_group'] = 'master';
                    }
                    break;
                }
