...
class ControllerCatalogProduct extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			// Заполним поля, которые нельзя менять с клиента
			$old_product = $this->model_catalog_product->getProductByModel($this->request->post['model']);
			$new_product = $this->request->post;
			if (!empty($old_product)){
				$new_product['image_uploaded_from_url'] = $old_product['image_uploaded_from_url'];
				$new_product['prime_cost'] = $old_product['prime_cost'];
				$new_product['import_history_id'] = $old_product['import_history_id'];
				$new_product['brand_name'] = $old_product['brand_name'];
			}

			$this->model_catalog_product->editProduct($this->request->get['product_id'], $new_product);
	...
	protected function getForm() {
		...
		foreach ($product_images as $product_image) {
			...
			$data['product_images'][] = array(
				...
				'image_uploaded_from_url' => $product_image['image_uploaded_from_url']
			);
		}
		...
		$data['available_city_list'] = $this->model_localisation_city->getCities();
		$data['cities'] = $this->model_catalog_product->getProductCities($this->request->get['product_id']);
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['model']) < 3) || (utf8_strlen($this->request->post['model']) > 512)) {
