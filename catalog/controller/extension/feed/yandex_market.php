...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_yandex_market_status')) {
			...
			if (isset($this->request->get['without_master_price'])){
				$without_master_price = $this->request->get['without_master_price'];
			} else {
				$without_master_price = 0;
			}
			...
			$categories = $this->model_extension_feed_yandex_market->getCategory($city_id);
			...
			$vendor_required = false; // true - только товары у которых задан магазин, необходимо для 'vendor.model'

			$this->load->model('account/customer_group');
			$MASTER_CUSTOMER_GROUP_NAME = 'Мастер';
			$customer_group_id = $this->model_account_customer_group->getCustomerGroupByName($MASTER_CUSTOMER_GROUP_NAME)['customer_group_id'];

			$products = $this->model_extension_feed_yandex_market->getProduct($allowed_categories, $allowed_manufacturers, $out_of_stock_id, $vendor_required, $bus_image, $bus_image_quantity, $bus_main_category, $bus_quantity_status, $city_id, $customer_group_id, $without_master_price, $category_id);
...
//				$data['cbid'] = 15;
				...
				if ($product['price'] == $product['master_price']){ // Нет скидки мастера
					...
					$data['price'] = number_format($this->currency->convert($this->tax->calculate($product['master_price'], $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
				}
...
//				$data['barcode'] = $product['sku'];
				...
				if (!empty($product['image'])) {
					...
					$data['picture'] = $this->getResizedImageName($product['image'], $bus_image_width, $bus_image_height);
				}

				if (isset($product['images'])) {
					foreach (explode(',', $product['images']) as $image) {
						// $data['picture'] .= ',' . $this->model_tool_image->resize($image, $bus_image_width, $bus_image_height);
						$data['picture'] .= ',' . $this->getResizedImageName($image, $bus_image_width, $bus_image_height);
					}
				}
/*
...
*/
	...
	private function getResizedImageName($filename, $width, $height){	
		$extension = pathinfo($filename, PATHINFO_EXTENSION);
		$image_new = utf8_substr($filename, 0, utf8_strrpos($filename, '.')) . '-' . (int)$width . 'x' . (int)$height . '.' . $extension;
		$image_new = str_replace(' ', '%20', $image_new);  // fix bug when attach image on email (gmail.com). it is automatic changing space " " to +

		return $this->url->link('common/cached_image', 'image=' . $image_new, $this->request->server['HTTPS']);
	...
	/**
	 * Формирование массива для элемента shop описывающего маркетплейс
	 *
	 ...
	 *		СВ - курс по банку той страны, к которой относится интернет-маркетплейс
