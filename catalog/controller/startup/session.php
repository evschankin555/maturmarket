...
class ControllerStartupSession extends Controller {
	public function index() {
		if (isset($this->request->get['api_token']) && isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/') {
			...
			// Убрал разлогинивание (делаем куку на 100 лет)
			setcookie($this->config->get('session_name'), $this->session->getId(), (time() + 3153600000), ini_get('session.cookie_path'), ini_get('session.cookie_domain'));
		}
	}
}