...
class ModelCatalogCms extends Model {
	...
	public function getProductRelatedByCategory($data) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related_wb pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.product_id = p.product_id) WHERE pr.category_id = '" . (int)$data['category_id'] . "' AND p.status = '1' AND p.date_available <= NOW() LIMIT " . (int)$data['limit']); 
	...
	public function getProductRelatedByManufacturer($data) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related_mn pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.product_id = p.product_id) WHERE pr.manufacturer_id = '" . (int)$data['manufacturer_id'] . "' AND p.status = '1' AND p.date_available <= NOW() LIMIT " . (int)$data['limit']); 
	...
	public function getTotalBestSeller() {
		$query = $this->db->query("SELECT COUNT(DISTINCT op.product_id) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW()");
	...
	public function getMostViewed($data = array()) {
		...
		INNER JOIN " . DB_PREFIX . "product_to_city p2ct ON (p.product_id = p2ct.product_id) 
		WHERE p.status = '1' AND p.date_available <= '" . $this->NOW . "' 
		and p2ct.city_id = '" . $this->session->data['city_id'] . "' 
