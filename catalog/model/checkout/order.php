...
if (!$product['prime_cost']){
	$this->log->write('1. prime_cost пустой для товара с product_id = ' . $product['product_id']);
}

				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', prime_cost = '" . (float)$product['prime_cost'] . "'");
...
if (!$product['prime_cost']){
	$this->log->write('2. prime_cost пустой для товара с product_id = ' . $product['product_id']);
}
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'shipment_date'           => $order_query->row['shipment_date'],
