...
class ModelExtensionFeedYandexMarket extends Model {
	public function getCategory($city_id = 0) {
		$query = $this->db->query("SELECT cd.name, c.category_id, c.parent_id FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND c.status = '1' AND c.sort_order <> '-1'" . ($city_id > 0 ? " and c.category_id in (select c2ct.category_id from " . DB_PREFIX . "category_to_city c2ct where c2ct.city_id = '" . (int)$city_id . "')" : ""));

		return $query->rows;
	}

	public function getProduct($allowed_categories, $allowed_manufacturers, $out_of_stock_id, $vendor_required = true, $bus_image = true, $bus_image_quantity = '9', $bus_main_category = true, $bus_quantity_status = false, $city_id = 0, $customer_group_id_for_special_price, $without_master_price = 0, $category_id = 0) {
		...
		$sql .= " m.name AS manufacturer, p.price, " . ($without_master_price ? "p.price" : "IFNULL(ps.price, p.price)") . " AS master_price FROM " . DB_PREFIX . "product_to_category AS p2c JOIN " . DB_PREFIX . "product AS p ON (p.product_id = p2c.product_id) " . ($vendor_required ? '' : 'LEFT ') . "JOIN " . DB_PREFIX . "manufacturer AS m ON (m.manufacturer_id = p.manufacturer_id) LEFT JOIN " . DB_PREFIX . "product_description AS pd ON (pd.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store AS p2s ON (p2s.product_id = p2c.product_id) LEFT JOIN " . DB_PREFIX . "product_special AS ps ON (ps.product_id = p2c.product_id) AND ps.customer_group_id = '" . (int)$customer_group_id_for_special_price . "' AND ps.date_start < NOW() AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())";
        ...
        $sql .= " AND p.status = '1'". ($city_id > 0 ? " and p.product_id in (select p2ct.product_id from " . DB_PREFIX . "product_to_city p2ct where p2ct.city_id = '" . (int)$city_id . "')" : "");
		...
		if ($category_id > 0) {
			$sql .= " AND p2c.category_id in (select cp.category_id from `" . DB_PREFIX . "category_path` cp WHERE cp.path_id = '" . (int)$category_id . "')";
		}
