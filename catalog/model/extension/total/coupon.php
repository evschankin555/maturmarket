...
class ModelExtensionTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			if ($coupon_query->row['total'] > $this->cart->getSubTotal()) {
				$status = false;
				$error_text = sprintf($this->language->get('error_coupon_small_total'), (int)$coupon_query->row['total'] . ' руб.');
			}
			...
			if ($coupon_query->row['uses_total'] > 0 && ($coupon_total >= $coupon_query->row['uses_total'])) {
				$status = false;
				$error_text = $this->language->get('error_coupon_uses_total_reached');
			}

			if ($coupon_query->row['logged'] && !$this->customer->getId()) {
				$status = false;
				$error_text = $this->language->get('error_coupon_logged_need');
			}
			...
			if ($this->customer->getId()) {
				...
				if ($coupon_query->row['uses_customer'] > 0 && ($customer_total >= $coupon_query->row['uses_customer'])) {
					$status = false;
					$error_text = sprintf($this->language->get('error_coupon_uses_customer_reached'), $coupon_query->row['uses_customer']);
				}

				// Проверим по группе промокода - в группе не должно быть уже использованных промокодов
				if (!empty($coupon_query->row['once_in_group'])){
					$once_in_group_count = $this->getCountCouponHistoriesByGroupAndCustomerId($coupon_query->row['once_in_group'], $this->customer->getId());
					if ($once_in_group_count){
						$status = false;
						$error_text = sprintf($this->language->get('error_coupon_once_in_group_reached'), $coupon_query->row['once_in_group']);
					}
				}
			}
			...
			if ($coupon_product_data || $coupon_category_data) {
				...
				if (!$product_data) {
					$status = false;
					$error_text = $this->language->get('error_coupon_product_not_found');
				}
			}
		} else {
			$status = false;
			$error_text = $this->language->get('error_coupon_coupon_not_found');
		}
		...
		if ($status) {
			return array(
				...
				'limit_for_percent' => $coupon_query->row['limit_for_percent'],
	...
	public function getTotal($total) {
		if (isset($this->session->data['coupon'])) {
			...
			if ($coupon_info) {
				...
				if ($coupon_info['type'] == 'P' and $coupon_info['limit_for_percent'] > 0) {
					$discount_total = min($discount_total, $coupon_info['limit_for_percent']);
				}
		...
		if ($code) {
			...
			if ($coupon_query->num_rows) {
				...
				if ($order_info['customer_id']) {
					...
					// Проверим по группе промокода - в группе не должно быть уже использованных промокодов
					if (!empty($coupon_query->row['once_in_group'])){
						$once_in_group_count = $this->getCountCouponHistoriesByGroupAndCustomerId($coupon_query->row['once_in_group'], $order_info['customer_id']);
						if ($once_in_group_count){
							$status = false;
						}
					}
	...
	public function getCountCouponHistoriesByGroupAndCustomerId($once_in_group, $customer_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch LEFT JOIN `" . DB_PREFIX . "coupon` c ON (ch.coupon_id = c.coupon_id) WHERE c.once_in_group = '" . $this->db->escape($once_in_group) . "' AND ch.customer_id = '" . (int)$customer_id . "'");
